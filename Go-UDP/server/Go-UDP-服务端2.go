...
//消息处理
func HandleMessage2(udpListener *net.UDPConn) {
	...
	if n > 0 {
		...
		default:
			...
			//群发，自己不处理
			for i := 0; i < len(onlineUser); i++ {
				if onlineUser[i].userListenConn == udpListener {
					continue
				}
				onlineUser[i].userListenConn.Write([]byte(onlineUser[i].userName + " " + nowHour + ":" + nowMinute + ":" + nowSecond + "\n" + msg[0]))
			}
			//if onlineUser[index].userAddr != addr {
